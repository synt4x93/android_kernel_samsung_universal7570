...
static int kbase_api_slsi_secure_flag_ctrl(struct kbase_context *kctx,
       struct kbase_ioctl_slsi_secure_flag *flags)
{
	if (!kctx || !flags)
		return -EINVAL;

#ifdef CONFIG_MALI_EXYNOS_SECURE_RENDERING
	return gpu_vendor_secure_rendering_dispatch(kctx, flags);
#else
	dev_err(kctx->kbdev->dev, "Mali: SecureRendering: IOCTL() called by wrong sequence\n");
	return 0;
#endif
}
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	/* MALI_SEC_SECURE_RENDERING */
	case KBASE_IOCTL_SLSI_SECURE_FLAGS:
		KBASE_HANDLE_IOCTL_IN(KBASE_IOCTL_SLSI_SECURE_FLAGS,
				kbase_api_slsi_secure_flag_ctrl,
				struct kbase_ioctl_slsi_secure_flag);
...
static int kbasep_protected_mode_init(struct kbase_device *kbdev)
{
	...
	kbdev->protected_ops->secure_mem_init(kbdev);
#endif
	return 0;
}
...
#endif /* if !defined(CONFIG_MALI_EXYNOS_SECURE_RENDERING) */
...
static int kbase_device_runtime_idle(struct device *dev)
{
	...
	/* MALI_SEC_INTEGRATION */
	/* We don't want the device to be suspended */
	/* It'll be controlled through power on/off sequence */
	return 1;
}
#endif /* KBASE_PM_RUNTIME */
